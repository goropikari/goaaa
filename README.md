...
```bash
go install github.com/goropikari/goaaa/cmd/goaaa@latest
```
...
```bash
goaaa ./path/to/example_test.go
goaaa ./path/to/package
```

変更された Go ファイルだけを検査する場合は `diff` サブコマンドを使います。引数は通常の `git diff` と同じように、range とパスを指定できます。
...
```bash
goaaa --format sarif ./path/to/package > results.sarif
```
